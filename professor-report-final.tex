...
ranking operativo su Chittagong. I tre controlli finali falliscono per motivi
diversi: temperature scaling non cambia le decisioni a soglia 0.5, Youden
oracle non e' deployabile, e la soglia trasferita da RSNA validation recupera
parzialmente il livello di \balacc{} ma non il ranking. La lettura finale e'
che la selezione del modello in trasferimento cross-domain richiede dati
target-domain, o almeno una validation rappresentativa del dominio di deploy.
E' stata inoltre aggiunta una prima estensione multi-classe
...
\texttt{outputs/predictions/\{dataset\}/\{model\}.npz}. Ogni file contiene
label, probabilita' della classe positiva \pneumonia{}, logit pre-sigmoid e
identificativo del campione. Il logit e' il valore prodotto dal modello prima
della sigmoid; la probabilita' e' ottenuta applicando la sigmoid una sola volta.
La soglia di Youden e' calcolata sullo stesso test set riportato:
e' quindi una soglia \emph{oracle}, ottimistica e non deployabile. Serve solo a
separare il potere discriminativo dal punto operativo fisso 0.5.
...
Il gap cross-dataset a soglia Youden segue molto da vicino il gap di AUC:
\(|\Delta(\mathrm{BA}_{Youden})-\Delta(\mathrm{AUC})| \leq 0.021\) per tutte
le cinque architetture. Con \(n=5\) modelli questo va letto come pattern
osservato robusto, non come inferenza statistica. Il segno e' pero' informativo:
...
canonica. La classe positiva e' sempre \pneumonia{}=1. L'ECE equal-width usa 10
bin uniformi in \([0,1]\); l'ECE equal-mass ordina gli score e divide i campioni
in 10 gruppi di cardinalita' simile. Il secondo schema e' utile quando molti
score sono saturati vicino a 1.0, perche' evita che la massa finisca quasi
tutta nello stesso bin. Il Brier score e' riportato come controllo
binning-free.
...
La verifica falsificabile sull'ipotesi di calibrazione e' positiva solo in
parte. Chittagong ha ECE equal-mass maggiore di RSNA per tutti i cinque modelli:
...
Per ogni modello sono stati generati anche reliability diagram separati per
RSNA, Kermany e Chittagong in
\texttt{outputs/calibration/reliability\_\{model\}.png}. Nel caso MobileNetV3
su RSNA sono presenti probabilita' saturate a 1.0, ma ECE equal-mass 0.023 e
Brier 0.042 indicano che la saturazione non e' di per se' un errore: il modello
e' molto confidente soprattutto su esempi corretti. Per questo il Brier score
viene usato come controllo rispetto a possibili artefatti del binning ECE.
...
applicata senza ri-fittare ai test RSNA, Kermany e Chittagong. Il protocollo e'
quindi deployabile rispetto ai test set: il test non viene usato per scegliere
la temperatura. La trasformazione e'
\[
p_T(y=1 \mid x) = \sigma(z(x)/T),
\]
dove \(z(x)\) e' il logit pre-sigmoid. Poiche' \(T>0\), la trasformazione e'
monotona e non puo' cambiare la ROC-AUC. Questa invarianza e' stata verificata
nel codice con un assert numerico.
...
Il risultato e' asimmetrico. Nel dominio Chittagong, il temperature scaling
migliora ECE equal-mass e Brier per ResNet18, MobileNetV3 ed EfficientNet-B0;
e' quasi neutro per PneumoniaNet; peggiora DenseNet121. Il caso DenseNet121 non e' un
artefatto del binning ECE: anche il Brier peggiora da 0.1756 a 0.1809. Inoltre
la temperatura fittata su RSNA validation e' \(T=0.851<1\), quindi rende le
probabilita' piu' estreme; questa correzione aiuta su RSNA ma peggiora su
Chittagong, confermando DenseNet121 come outlier di dominio.

Il temperature scaling non puo' invece correggere il ranking a soglia fissa
0.5. Infatti, per ogni \(T>0\),
\[
\sigma(z/T) \geq 0.5 \Longleftrightarrow z \geq 0.
\]
Le predizioni binarie a soglia 0.5 restano quindi identiche prima e dopo la
calibrazione. Numericamente, su Chittagong la correlazione di rango tra ranking
a soglia 0.5 e ranking oracle Youden resta 0.10 sia prima sia dopo il
temperature scaling.

E' stata infine provata una variante deployabile di scelta della soglia: per
ogni modello si calcola la soglia Youden su RSNA validation e la si applica a
Chittagong. Anche questa procedura non usa label di test Chittagong, quindi e'
deployabile rispetto al test esterno. Il confronto con la soglia Youden oracle
su Chittagong e' riportato nella tabella seguente.
...
checkpoint. Lo script di scaling chiama il training loop per tutti i punti e non
ricarica esplicitamente i checkpoint del benchmark principale. Tuttavia gli
artefatti prodotti mostrano due casi diversi. Il punto PneumoniaNet \(w=1.0\)
ha la stessa architettura della PneumoniaNet del benchmark principale, ma non lo
stesso checkpoint: il benchmark seleziona il best checkpoint a epoca 5, mentre
la run di scaling \(w=1.0\) seleziona il best checkpoint a epoca 16. Questo
spiega la discrepanza numerica tra le due tabelle per la stessa architettura:
0.896 contro 0.914 su RSNA e 0.830 contro 0.756 su Chittagong. Per
EfficientNet-B0, invece, la riesecuzione con lo stesso seed seleziona lo stesso
best checkpoint del benchmark principale, con metriche identiche alle cifre
riportate. Questa asimmetria e' un esito empirico degli artefatti, non una
differenza intenzionale di protocollo. Dai metadata dei checkpoint non emerge
una differenza di \texttt{epochs}, \texttt{patience}, learning rate o seed; la
divergenza del best epoch implica quindi che il training non e' bit-identico
tra esecuzioni separate. Rimane non-determinismo residuo nella pipeline, per
esempio nello shuffling del DataLoader e/o nei kernel numerici usati dal backend. L'asimmetria tra i due modelli e' coerente con la curva di validation piatta di PneumoniaNet su RSNA (\balacc{} tra 0.914 e 0.924): quando il massimo della val-BA e' quasi piatto tra epoche, una perturbazione non deterministica sposta facilmente l'argmax (epoca 5 contro 16), mentre per EfficientNet-B0 il picco e' piu' netto e la selezione resta stabile.
I valori assoluti delle due tabelle non devono quindi essere usati come se tutti
i punti fossero la stessa identica esecuzione; per PneumoniaNet il riferimento
assoluto resta il checkpoint del benchmark principale, mentre la tabella di
scaling serve soprattutto a leggere l'andamento entro-protocollo.
...
0.946, B2 0.947 e B3 0.936. La curva interna risulta quindi gia' quasi piatta
nel range esplorato, entro la variabilita' del training, piu' che un plateau
monotono pulito.
...
misurare la variabilita'. Lo stesso script puo' essere rilanciato piu' volte
variando \texttt{--seed} e cambiando la dimensione del training set tramite
...
esterni non permettono lo stesso controllo sistematico. L'analisi a soglia
Youden mostra inoltre che la difficolta' relativa dei due dataset esterni non e'
uniforme per tutte le architetture: quattro modelli su cinque hanno
\balacc{} oracle piu' alta su Chittagong che su Kermany, mentre DenseNet121
inverte il segno. La view position va quindi trattata come ipotesi da
controllare, non come meccanismo sufficiente a spiegare il trasferimento.
...
L'analisi della soglia falsifica la spiegazione piu' semplice del caso
DenseNet121. Se il gap Kermany--Chittagong fosse principalmente un effetto del
punto operativo 0.5, ci si aspetterebbe che il gap si chiudesse passando alla
soglia Youden oracle. Invece DenseNet121 passa da un gap di \balacc{} 0.094 a un
gap oracle 0.088, molto vicino al gap di AUC 0.081. Quindi, per DenseNet121, il
divario tra Kermany e Chittagong riflette soprattutto separabilita' diversa tra
i due domini, non solo una soglia mal posizionata.
...
Temperature scaling, fittato correttamente su RSNA validation e applicato ai
test, migliora la calibrazione nel dominio Chittagong per ResNet18, MobileNetV3
ed EfficientNet-B0, ma non per PneumoniaNet e DenseNet121. Questo statement e'
specifico di Chittagong: su RSNA, ad esempio, PneumoniaNet e DenseNet121
migliorano leggermente dopo temperature scaling. Il caso DenseNet121 su
Chittagong e'
particolarmente importante: ECE equal-mass peggiora da 0.120 a 0.131 e anche il
Brier score peggiora da 0.1756 a 0.1809, quindi il peggioramento non e' un
artefatto del binning ECE.

La calibrazione tramite temperatura non puo' cambiare il ranking a soglia 0.5:
essendo una trasformazione monotona dei logit con \(T>0\), lascia invariato il
segno del logit e quindi le decisioni binarie a soglia 0.5. Numericamente, su
Chittagong la Spearman tra ranking a soglia 0.5 e ranking Youden oracle resta
0.10 prima e dopo temperature scaling. Anche la soglia Youden fittata su RSNA
validation e applicata a Chittagong resta lontana dal ranking oracle
(\(\rho=0.20\)). La conclusione e' quindi precisa: la miscalibrazione contribuisce
al problema, ma una calibrazione globale source-domain non rimuove lo shift
...
\begin{itemize}
  ...
  \item \textbf{Guo et al.} \cite{guo2017calibration}
  (\url{https://arxiv.org/abs/1706.04599}): motiva il temperature scaling come
  calibrazione post-hoc dei logit su un set di validation separato. Nel lavoro
  qui riportato la temperatura e' fittata su RSNA validation e applicata senza
  ri-fittare ai test RSNA, Kermany e Chittagong.
\end{itemize}
